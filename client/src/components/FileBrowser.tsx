  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-xl max-w-lg w-full max-h-[80vh] flex flex-col">
        ...
        {/* Breadcrumb */}
        <div className="px-4 py-2 bg-gray-50 dark:bg-gray-700 border-b border-gray-200 dark:border-gray-700 text-sm flex items-center gap-1 overflow-x-auto flex-shrink-0">
          <button onClick={() => loadPath()} className="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 flex-shrink-0">
            Root
          </button>
          {pathSegments.map((segment, idx) => {
            const parts = pathSegments.slice(0, idx + 1);
            const fullPath = isWindows ? parts.join('\\') + '\\' : '/' + parts.join('/');
            return (
              <span key={idx} className="flex items-center gap-1 flex-shrink-0">
                <span className="text-gray-400">/</span>
                <button onClick={() => loadPath(fullPath)} className="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300">
                  {segment}
                </button>
              </span>
            );
        ...
        <div className="px-4 py-3 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700 flex items-center justify-between">
          <div className="text-xs text-gray-500 dark:text-gray-400 font-mono truncate max-w-[60%]">
